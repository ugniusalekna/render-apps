...
def plot_gradient_descent(initial_point, learning_rate):
    ...
    path = gradient_descent(function, gradient, initial_point=initial_point, learning_rate=learning_rate)[::-1]
    ...
    layout = go.Layout(
        title={
            'text': f"Gradient Descent (lr={learning_rate:.3f}, start={initial_point:.3f})",
            'x': 0.5,
            'xanchor': 'center',
            'y': 0.95,
            'yanchor': 'bottom'
...
def create_app():
    ...
    app = Dash(__name__, routes_pathname_prefix='/gradient-descent/')
        ...
        style={'backgroundColor': '#f9f9f9', 'padding': '20px', 'max-width': '1200px', 'margin': '0 auto', 'font-family': 'Arial, sans-serif'}, children=[
            ...
            html.Div(style={'display': 'flex', 'justify-content': 'space-around', 'margin-bottom': '10px'}, children=[
                html.Div(style={'border': '2px solid #ddd', 'padding': '10px', 'border-radius': '10px', 'width': '45%'}, children=[
                    ...
                    dcc.Slider(id='learning-rate', min=0.001, max=0.1, step=0.001, value=0.05,  marks={i/100: f"{i/100:.2f}" for i in range(2, 10, 2)}, tooltip={"placement": "bottom", "always_visible": True}),
                ])
            ]),
    ])
